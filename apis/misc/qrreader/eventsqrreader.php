    ...
    if($_SERVER["REQUEST_METHOD"] === "GET" && $inno_id !== "" && md5($verifyCode)===$code) {
        ...
        else
        {
            while($row = mysqli_fetch_array($query1, MYSQL_ASSOC))
            {
                if($row['checked_in'] == 1)
                {
                    $query2 = "UPDATE events_registration SET checked_in = 1 WHERE inno_id =".mysqli_real_escape_string($conn, $inno_id)." AND event_id =".mysqli_real_escape_string($conn, $eid);
                ...
                {
                    echo json_encode(array('status'=>'failure', 'result' => "Student hasn't checked into the Institute."));
                }
            }
        }
    }
